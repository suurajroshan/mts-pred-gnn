            ...
            if 'pretext' in opt.model:
                ...
                for x, stamp, y in data_iter:
                    ...
                    y_pred = predict_stamp(model, x, stamp, y, opt).permute(
                        0, 3, 2, 1).reshape(-1, opt.n_route)

                    y_pred = scaler.inverse_transform(
                        y_pred.cpu().numpy()).reshape(-1, 1, opt.n_pred, opt.n_route)
                    y = scaler.inverse_transform(
                        y.permute(0, 3, 2, 1).reshape(-1, opt.n_route).cpu().numpy()).reshape(-1, 1, opt.n_pred, opt.n_route)
