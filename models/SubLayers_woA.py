...
class TPGNN(nn.Module):
    ...
    def forward(self, x, stamp):
        ...
        #adj = self.distant_mat.clone() #commented
        # adj_2 = self.cor_mat.clone()
        if self.training:
            nonzero_mask = self.distant_mat > 0
            #adj[nonzero_mask] = F.dropout(adj[nonzero_mask], p=self.droprate) # commented

        #adj_1 = torch.softmax(torch.relu(laplacian(adj))/self.temperature, dim=0)
        #adj_2 = torch.softmax(torch.relu(self.r1@self.r1.T)/self.temperature, dim=0) # commented
        #adj_1 = F.dropout(adj_1, p=self.droprate) #c

        #adj_2 = F.dropout(adj_2, p=self.droprate) #c
        # (b,n,t,k)->(b,t,n,k)
        ...
        for i in range(1, self.kt):
            #z = adj_1@z + \
             #   (x@w_stack[i]*(temp_1[:, i].reshape(b, 1, 1, 1))
              #   ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
                z = (x@w_stack[i]*(temp_1[:, i].reshape(b, 1, 1, 1))
                 ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
        ...
        for i in range(1, self.kt):
            #z_fix = adj_2@z_fix + \
             #   (x@w_stack[i]*(temp_1[:, i].reshape(b, 1, 1, 1))
              #   ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
            z_fix = (x@w_stack[i]*(temp_1[:, i].reshape(b, 1, 1, 1))
                 ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
        z = z+z_fix
