...
class WOTPG(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat, kt=2, n_c=10, droprate=0., temperature=1.0) -> None:
        super(WOTPG, self).__init__()
        self.droprate = droprate
        print(n_route, n_c)
        self.r1 = nn.Parameter(torch.randn(n_route, n_c))
        # self.r2 = nn.Parameter(torch.randn(n_route, 10))
...
class WODynamic(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat, kt=2, n_c=10, droprate=0., temperature=1.0) -> None:
        super(WODynamic, self).__init__()
        self.droprate = droprate
        print(n_route, n_c)
        self.r1 = nn.Parameter(torch.randn(n_route, n_c))
        # self.r2 = nn.Parameter(torch.randn(n_route, 10))
        self.w_stack = nn.Parameter(torch.randn(kt+1, d_attribute, d_out))
        nn.init.xavier_uniform_(self.w_stack.data)
        self.static = nn.Parameter(torch.randn(kt+1))
    ...
    def forward(self, x, stamp):
        ...
        for i in range(1, self.kt):
            z = adj_1@z + \
                (x@w_stack[i]*self.static[i]
                 ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
        z_fix = (x@w_stack[0])*self.static[0]
        z_fix = z_fix.permute(0, 2, 1, 3).reshape(b*t, n, -1)
        for i in range(1, self.kt):
            z_fix = adj_2@z_fix + \
                (x@w_stack[i]*self.static[i]
...
class WOOverview(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat, kt=2, n_c=10, droprate=0., temperature=1.0) -> None:
        super(WOOverview, self).__init__()
        self.droprate = droprate
        print(n_route, n_c)
        self.r1 = nn.Parameter(torch.randn(n_route, n_c))
        # self.r2 = nn.Parameter(torch.randn(n_route, 10))
        self.w_stack = nn.Parameter(torch.randn(kt+1, d_attribute, d_out))
        nn.init.xavier_uniform_(self.w_stack.data)
        self.reduce_stamp = nn.Linear(n_his, n_his, bias=False)
        self.temp_1 = nn.Linear(d_attribute//4, kt+1)
        # self.temp_2 = nn.Linear(d_attribute//4, kt+1)
        self.temperature = temperature
        self.d_out = d_out
    ...
    def forward(self, x, stamp):
        ...
        # print(stamp.shape)
        # (b,t,k)->(b,k,1)->(b,kt+1)
        period_emb = self.reduce_stamp(stamp.permute(0, 2, 1))
        period_emb = period_emb.permute(0,2,1)
        temp_1 = self.temp_1(period_emb)
        # temp_2 = self.temp_2(period_emb)
        adj = self.distant_mat.clone()
        # adj_2 = self.cor_mat.clone()
        ...
        temp_expanded = temp_1[:,:,0].unsqueeze(1).unsqueeze(3)
        temp_expanded = temp_expanded.expand_as(x)
        z = torch.mul(x@w_stack[0], temp_expanded)
        z = z.permute(0, 2, 1, 3).reshape(b*t, n, -1)
        # for i in range(1, self.kt):
        #     z = adj@z + \
        #         (x@w_stack[i]*(temp[:, i].reshape(b, 1, 1, 1))
        #          ).permute(0, 2, 1, 3).reshape(b*t, n, -1)
        for i in range(1, self.kt):
            temp_expanded = temp_1[:, :, i].unsqueeze(1).unsqueeze(3)
            temp_expanded = temp_expanded.expand_as(x)
            z_add = torch.mul(x @ w_stack[i], temp_expanded)
            z = adj_1@z + z_add.permute(0, 2, 1, 3).reshape(b*t, n, -1)

        temp_expanded = temp_1[:, :, 0].unsqueeze(1).unsqueeze(3)
        temp_expanded = temp_expanded.expand_as(x)
        z_fix = torch.mul(x @ w_stack[0], temp_expanded)
        z_fix = z_fix.permute(0, 2, 1, 3).reshape(b*t, n, -1)
        for i in range(1, self.kt):
            temp_expanded = temp_1[:, :, i].unsqueeze(1).unsqueeze(3)
            temp_expanded = temp_expanded.expand_as(x)
            z_add = torch.mul(x @ w_stack[i], temp_expanded)
            z_fix = adj_2@z_fix + z_add.permute(0, 2, 1, 3).reshape(b*t, n, -1)
        z = z+z_fix
...
class WONormalize(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat, kt=2, n_c=10, droprate=0., temperature=1.0) -> None:
        super(WONormalize, self).__init__()
        self.droprate = droprate
        print(n_route, n_c)
        self.r1 = nn.Parameter(torch.randn(n_route, n_c))
        # self.r2 = nn.Parameter(torch.randn(n_route, 10))
...
class WOKmatrics(nn.Module):
    # softlap+outerwave
    def __init__(self, d_attribute, d_out, n_route, n_his, dis_mat, kt=2, n_c=10, droprate=0., temperature=1.0) -> None:
        super(WOKmatrics, self).__init__()
        self.droprate = droprate
        print(n_route, n_c)
        self.r1 = nn.Parameter(torch.randn(n_route, n_c))
        # self.r2 = nn.Parameter(torch.randn(n_route, 10))
